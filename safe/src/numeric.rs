...
pub(crate) unsafe fn errno_location() -> *mut c_int {
    errno_ptr()
}
...
pub(crate) unsafe fn json_parse_double_len_impl(
    buf: *const c_char,
    len: c_int,
    retval: *mut c_double,
) -> c_int {
    if buf.is_null() || retval.is_null() || len < 0 {
        return 1;
    }

    let mut end = ptr::null_mut();
    *retval = strtod(buf, &mut end);
    if end == buf.cast_mut().add(len as usize) {
        0
    } else {
        1
    }
