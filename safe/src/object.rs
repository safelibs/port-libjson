...
{
    let table =
        linkhash::lh_kchar_table_new_impl(JSON_OBJECT_DEF_HASH_ENTRIES, Some(json_object_lh_entry_free));
    if table.is_null()
    {
        set_errno(ENOMEM);
        return ptr::null_mut();
...
pub(crate) unsafe fn json_object_set_boolean_impl(obj: *mut json_object, value: json_bool) -> c_int
{
...
pub(crate) unsafe fn json_object_get_int_impl(obj: *const json_object) -> int32_t
{
    ...
    match &inner.data
    {
        ...
        {
            let capped = if *value >= i64::MAX as u64 { i64::MAX } else { *value as i64 };
        ...
        JsonData::String(_) =>
        {
            ...
            if crate::numeric::json_parse_int64_impl(json_object_get_string_impl(obj.cast_mut()), &mut parsed) != 0
            {
                return 0;
            }
...
pub(crate) unsafe fn json_object_get_uint64_impl(obj: *const json_object) -> uint64_t
{
    ...
    match &inner.data
    {
        ...
        JsonData::String(_) =>
        {
            ...
            if crate::numeric::json_parse_uint64_impl(json_object_get_string_impl(obj.cast_mut()), &mut parsed) != 0
            {
                return 0;
            }
            parsed
...
{
    string_bytes(obj).map(|bytes| bytes.len() as c_int).unwrap_or(0)
}

pub(crate) unsafe fn json_object_set_string_impl(obj: *mut json_object, value: *const c_char) -> c_int
{
    if value.is_null()
    {
...
pub(crate) unsafe fn json_object_array_shrink_impl(obj: *mut json_object, empty_slots: c_int) -> c_int
{
    if empty_slots < 0
    {
        return -1;
...
pub(crate) unsafe fn json_object_equal_impl(left: *mut json_object, right: *mut json_object) -> c_int
{
    if left == right
    {
        return 1;
    ...
    let equal = match (&left_inner.data, &right_inner.data)
    {
        ...
        (JsonData::String(a), JsonData::String(b)) =>
        {
            strings_equal(&a[..a.len().saturating_sub(1)], &b[..b.len().saturating_sub(1)])
        }
...
) -> c_int
{
    ...
    let rc = json_object_deep_copy_recursive(src, ptr::null_mut(), ptr::null(), usize::MAX, dst, shallow);
    if rc < 0
    {
...
impl<'a> Parser<'a>
{
    ...
    unsafe fn parse(mut self) -> Option<*mut json_object>
    {
        self.skip_ws();
    ...
    unsafe fn parse_value(&mut self) -> Option<*mut json_object>
    {
        self.skip_ws();
        match self.peek()?
        {
            b'{' => self.parse_object(),
            b'[' => self.parse_array(),
            b'"' => Some(self.parse_string_object()?),
            b't' => self.parse_literal(b"true", json_object_new_boolean_impl(1)),
            b'f' => self.parse_literal(b"false", json_object_new_boolean_impl(0)),
            b'n' => self.parse_literal(b"null", ptr::null_mut()),
    ...
    unsafe fn parse_literal(
        ...
        literal: &[u8],
        value: *mut json_object,
    ) -> Option<*mut json_object>
    {
        if self.input.get(self.offset..self.offset + literal.len())? != literal
        {
            return None;
        }
    ...
    unsafe fn parse_object(&mut self) -> Option<*mut json_object>
    {
        debug_assert_eq!(self.bump(), Some(b'{'));
        let obj = json_object_new_object_impl();
        if obj.is_null()
        {
            return None;
        }
        ...
        loop
        {
            ...
            let key = match CString::new(key_bytes)
            {
                Ok(key) => key,
                Err(_) =>
                {
            ...
            if json_object_object_add_impl(obj, key.as_ptr(), value) < 0
            {
                json_object_put_impl(value);
                json_object_put_impl(obj);
                return None;
    ...
    unsafe fn parse_array(&mut self) -> Option<*mut json_object>
    {
        debug_assert_eq!(self.bump(), Some(b'['));
        let array = json_object_new_array_impl();
        if array.is_null()
        {
            return None;
        }
        ...
        loop
        {
            let value = match self.parse_value()
            {
                ...
                None =>
                {
                    json_object_put_impl(array);
                    return None;
                }
            };
            if json_object_array_add_impl(array, value) < 0
            {
                json_object_put_impl(value);
                json_object_put_impl(array);
                return None;
    ...
    unsafe fn parse_string_object(&mut self) -> Option<*mut json_object>
    {
        let bytes = self.parse_string_bytes()?;
        Some(new_json_object(6, JsonData::String(with_nul(&bytes))))
    }

    fn parse_hex_digit(byte: u8) -> Option<u16>
    {
        match byte
        {
    ...
    fn parse_u16_escape(&mut self) -> Option<u16>
    {
        let mut value = 0_u16;
        for _ in 0..4
        {
            value = (value << 4) | Self::parse_hex_digit(self.bump()?)?;
        }
        Some(value)
    }
                ...
                b'\\' => match self.bump()?
                {
                    ...
                    b'u' =>
                    {
                        ...
                        if (0xD800..=0xDBFF).contains(&first) &&
                            self.peek() == Some(b'\\') &&
                            self.input.get(self.offset + 1) == Some(&b'u')
                        {
                            self.offset += 2;
                            ...
                            Self::push_utf8(&mut out, codepoint);
                        }
                        else
                        {
                            Self::push_utf8(&mut out, first as u32);
                        }
                    }
                    _ => return None,
                },
    ...
    unsafe fn parse_number(&mut self) -> Option<*mut json_object>
    {
        ...
        let c_token = CString::new(token).ok()?;
        if is_double
        {
            let value: f64 = std::str::from_utf8(token).ok()?.parse().ok()?;
            return Some(json_object_new_double_s_impl(value, c_token.as_ptr()));
        }

        if token.first() == Some(&b'-')
        {
            if let Ok(value) = std::str::from_utf8(token).ok()?.parse::<i64>()
            {
                return Some(json_object_new_int64_impl(value));
            }
        }
        else
        {
            ...
            if let Ok(value) = std::str::from_utf8(token).ok()?.parse::<u64>()
            {
                return Some(json_object_new_uint64_impl(value));
            }
        }

        let value: f64 = std::str::from_utf8(token).ok()?.parse().ok()?;
        Some(json_object_new_double_s_impl(value, c_token.as_ptr()))
    }
}
...
pub(crate) unsafe fn json_tokener_parse_impl(text: *const c_char) -> *mut json_object
{
    ...
    Parser::new(input).parse().unwrap_or(ptr::null_mut())
}
