...
fn append_bytes(pb: *mut printbuf, bytes: &[u8]) -> c_int {
    printbuf_impl::printbuf_memappend_impl(pb, bytes.as_ptr().cast(), bytes.len() as c_int)
}
...
fn call_serializer(
    serializer: json_object_to_json_string_fn,
    obj: *mut json_object,
    pb: *mut printbuf,
    level: c_int,
    flags: c_int,
) -> c_int {
    unsafe { serializer(obj, pb, level, flags) }
}

fn call_delete_fn(delete_fn: json_object_delete_fn, obj: *mut json_object, userdata: *mut c_void) {
    unsafe {
        delete_fn(obj, userdata);
    }
}

fn write_length_out(length_out: *mut size_t, value: size_t) {
    unsafe {
        *length_out = value;
    }
}

fn printbuf_bpos(pb: *mut printbuf) -> size_t {
    unsafe { (*pb).bpos as size_t }
}

fn printbuf_buf(pb: *mut printbuf) -> *const c_char {
    unsafe { (*pb).buf.cast() }
}
...
fn c_bytes_with_nul(value: *const c_char) -> Vec<u8> {
    unsafe { CStr::from_ptr(value).to_bytes_with_nul().to_vec() }
}

fn free_userdata(userdata: *mut c_void) {
    unsafe {
        free(userdata);
    }
}

fn format_snprintf(buffer: &mut [c_char], format: &[u8], value: c_double) -> c_int {
    unsafe {
        snprintf(
            buffer.as_mut_ptr(),
            buffer.len(),
...
fn rendered_buffer_bytes(buffer: &[c_char]) -> Vec<u8> {
    unsafe { CStr::from_ptr(buffer.as_ptr()).to_bytes().to_vec() }
}
...
fn serialize_child(
    ...
    let serializer = inner
        .to_json_string
        .expect("json_object serializer must be set");
    call_serializer(serializer, child, pb, level, flags)
}

pub(crate) fn json_object_get_userdata_impl(obj: *mut json_object) -> *mut c_void {
    object::as_json_box(obj)
        .map(|inner| inner.userdata)
        .unwrap_or(ptr::null_mut())
}

pub(crate) fn json_object_set_userdata_impl(
    obj: *mut json_object,
    userdata: *mut c_void,
    ...
    if let Some(delete_fn) = inner.user_delete {
        call_delete_fn(delete_fn, obj, inner.userdata);
    }
    inner.userdata = userdata;
    inner.user_delete = user_delete;
}

pub(crate) fn json_object_set_serializer_impl(
    obj: *mut json_object,
...
pub(crate) fn json_object_to_json_string_length_impl(
    obj: *mut json_object,
    flags: c_int,
    ...
    let serializer = inner
        .to_json_string
        .expect("json_object serializer must exist");
    if call_serializer(serializer, obj, inner.pb, 0, flags) < 0 {
        if !length_out.is_null() {
            write_length_out(length_out, 0);
        }
        return ptr::null();
    ...
    if !length_out.is_null() {
        write_length_out(length_out, printbuf_bpos(inner.pb));
    }
    printbuf_buf(inner.pb)
}

pub(crate) fn json_object_to_json_string_ext_impl(
    obj: *mut json_object,
    flags: c_int,
) -> *const c_char {
...
pub(crate) fn json_object_to_json_string_impl(obj: *mut json_object) -> *const c_char {
...
pub(crate) unsafe extern "C" fn json_object_boolean_to_json_string_impl(
    ...
    match inner.data {
        object::JsonData::Boolean(value) => {
            if value != 0 {
                append_literal(pb, TRUE_BYTES)
            } else {
                append_literal(pb, FALSE_BYTES)
            }
        }
        _ => -1,
    }
...
fn current_double_format() -> Option<Vec<u8>> {
    THREAD_DOUBLE_FORMAT.with(|tls| {
        if let Some(bytes) = tls.borrow().as_ref() {
            ...
            GLOBAL_DOUBLE_FORMAT
                .lock()
                .expect("global format mutex poisoned")
                .clone()
        }
    })
}
...
fn format_double(value: c_double, explicit_format: *const c_char, flags: c_int) -> Option<Vec<u8>> {
    if value.is_nan() {
    ...
    let format_bytes = if explicit_format.is_null() {
        ...
        c_bytes_with_nul(explicit_format)
    };

    let mut buffer = [0_i8; 128];
    let rc = format_snprintf(&mut buffer, &format_bytes, value);
    if rc < 0 {
        return None;
    }

    let mut rendered = rendered_buffer_bytes(&buffer);
...
pub(crate) unsafe extern "C" fn json_object_userdata_to_json_string_impl(
    ...
    append_bytes(pb, &c_bytes(inner.userdata.cast()))
}
...
pub(crate) fn json_c_set_serialization_double_format_impl(
    ...
    let duplicated = if double_format.is_null() {
        ...
        Some(c_bytes_with_nul(double_format))
    };
    ...
    match global_or_thread {
        object::JSON_C_OPTION_GLOBAL => {
            ...
            *GLOBAL_DOUBLE_FORMAT
                .lock()
                .expect("global format mutex poisoned") = duplicated;
            0
        }
