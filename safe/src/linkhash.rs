...
pub(crate) unsafe fn lh_kchar_table_new_impl(
    ...
    lh_table_new_impl(
        size,
        free_fn,
        Some(active_char_hash_fn()),
        Some(lh_char_equal_impl),
    )
}
...
pub(crate) unsafe fn lh_table_resize_impl(t: *mut lh_table, new_size: c_int) -> c_int {
    ...
    while !entry.is_null() {
        ...
        if lh_table_insert_w_hash_impl(
            new_t,
            (*entry).k,
            (*entry).v,
            table_hash(new_t, (*entry).k),
            opts,
        ) != 0
        {
...
pub(crate) unsafe fn lh_table_insert_w_hash_impl(
    ...
    {
        let new_size = if (*t).size > c_int::MAX / 2 {
            c_int::MAX
        } else {
            (*t).size * 2
        };
...
pub(crate) unsafe fn lh_table_lookup_entry_impl(
    t: *mut lh_table,
    key: *const c_void,
) -> *mut lh_entry {
    if t.is_null() {
